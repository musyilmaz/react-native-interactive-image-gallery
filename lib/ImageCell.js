...
import { Image, Easing, Animated, Dimensions, StyleSheet, TouchableOpacity } from "react-native";
...
class ImageCell extends React.Component<Props, State> {
  ...
  measurePhoto = async () => {
    ...
    return new Promise((resolve: Function, reject: Function) => {
      this._imageRef
        .getNode()
        .measure(
          (imgX: number, imgY: number, imgWidth: number, imgHeight: number, imgPageX: number, imgPageY: number) => {
            resolve({
              width: imgWidth,
              height: imgHeight,
              x: imgPageX,
  ...
  measureImageSize = async () => {
    ...
    return new Promise((resolve: Function, reject: Function) => {
      Image.getSize(
        ...
        (error: Error) => {
          console.error("measureImageSize: Error trying to get image size", JSON.stringify(error.message));
          reject(error);
        }
      );
    });
  ...
  render() {
    return (
      <TouchableOpacity key={this.props.imageId} style={styles.cellContainer} onPress={this._onPressImage}>
        <Animated.Image
          ...
          style={[
            {
              ...
              height: ((Dimensions.get("window").width / 2) * 301) / 632,
